"""
radar_cross_section.py

RCS look-up table for a representative target at 5-degree azimuth intervals.
...
def get_rcs_dBsm(azimuth):
    """Return RCS in dBsm at the given azimuth angle (degrees, -180 to 180).

    Exact table entries are returned directly; azimuths between entries are
    linearly interpolated. The table has 5-degree resolution so the interpolation
    error is small for typical use.
    """
    data = {
...
def dbsm_to_m2(dbsm):
    """Convert RCS from dBsm to linear m^2."""
    return 10 ** (dbsm / 10)


def get_rcs_m2(azimuth):
    """Return interpolated RCS in m^2 for a given azimuth angle (degrees)."""
